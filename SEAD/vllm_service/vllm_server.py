#!/usr/bin/env python
# -*- coding: utf-8 -*-
# SEAD/vllm_service/vllm_server.py
...
class UserParamsManager:
    ...
    def get_batch_user_params(self, batch_size: int, is_validation: bool = False) -> list:
        ...
        if len(params_list) == 0:
            logger.warning(f"{split} user_params is empty")
            return [{}] * batch_size
...
try:
    ...
    if not hasattr(tokenizer, 'chat_template') or tokenizer.chat_template is None:
        logger.warning('Tokenizer does not have chat_template, will use fallback mode')
...
def fill_user_params(template: str, user_params: Dict[str, Any]) -> str:
    ...
    try:
        ...
        if random.random() < 0.05:
            ...
            if unfilled_dollar:
                logger.warning(f"[DEBUG] Unfilled ${{{...}}}: {unfilled_dollar}")
            if unfilled_brace:
                logger.warning(f"[DEBUG] Unfilled {{...}}: {unfilled_brace}")
...
def dialogue_batch():
    ...
    try:
        ...
        # Require batch_user_profiles; indexed user_params lookup is disabled.
        batch_user_profiles = request_data.get('batch_user_profiles', [])
        if not isinstance(batch_user_profiles, list) or len(batch_user_profiles) != batch_size:
            return jsonify({'error': 'batch_user_profiles is required and must match batch size; indexed user_params lookup is disabled'}), 400

        batch_user_params = []
        for raw in batch_user_profiles:
            if isinstance(raw, dict):
                batch_user_params.append(raw)
            else:
                try:
                    batch_user_params.append(json.loads(raw))
                except Exception:
                    batch_user_params.append({})
