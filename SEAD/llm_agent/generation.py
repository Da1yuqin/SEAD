...
class GenerationConfig:
    ...
    @property
    def chatbot_urls(self) -> List[str]:
        """Parse chatbot_url into a list of URLs (supports comma-separated multi-instance)."""
        urls = [u.strip() for u in self.chatbot_url.split(',') if u.strip()]
        if not urls:
...
class LLMGenerationManager:
    ...
    def _get_batch_user_profiles(self, gen_batch: DataProto, batch_size: int) -> List[Dict[str, Any]]:
        """
        Extract per-sample user profiles carried by the training batch.
        """
        raw_profiles = gen_batch.non_tensor_batch.get('user_profile')
        if raw_profiles is None:
            raise ValueError('user_profile key is missing in gen_batch.non_tensor_batch')
            # logger.warning('user_profile key is missing in gen_batch.non_tensor_batch, defaulting to empty profiles')
        ...
        for raw_profile in raw_profiles:
            if not isinstance(raw_profile, str):
                raise TypeError(
                    f"user_profile entry must be a JSON string, got {type(raw_profile).__name__!r}: {raw_profile!r}"
                )
                # logger.warning(f'Unexpected user_profile type {type(raw_profile).__name__!r}, defaulting to empty profile')
                # profiles.append({})
            parsed = json.loads(raw_profile)  # raises json.JSONDecodeError on bad data
            if not isinstance(parsed, dict):
                raise ValueError(
                    f"user_profile JSON must decode to a dict, got {type(parsed).__name__!r}: {raw_profile[:120]!r}"
                )
                # logger.warning(f'invalid user_profile format (not a dict) for entry: {raw_profile[:120]!r}, defaulting to empty profile')
                # profiles.append({})
            profiles.append(parsed)
        return profiles

    def _batch_call_chatbot(self, server_url: str, histories: List[List[Dict[str, str]]], turn_ids: List[int], batch_statuses: List[List[int]], batch_user_profiles: List[Dict] = None) -> List[Dict[str, Any]]:
        """Batch call user simulator API (supports multi-instance load balancing)."""

        batch_size = len(histories)
        max_retries = self.config.max_api_retries
        urls = self.config.chatbot_urls

        # Single instance: use original path
        if len(urls) == 1:
            return self._batch_call_single(urls[0], histories, turn_ids, batch_statuses, batch_user_profiles, max_retries)

        # Multi-instance: split batch evenly, call in parallel
        logger.info(f"Calling batch API: {batch_size} samples across {len(urls)} instances (mode={self.config.mode})")

        chunk_size = (batch_size + len(urls) - 1) // len(urls)
        chunks = []
        ...
        results_per_chunk = [None] * len(chunks)

        def _call_instance(idx: int, url: str, start: int, end: int):
            sub_turn_ids = turn_ids[start:end]
            sub_statuses = batch_statuses[start:end]
            try:
                sub_histories = histories[start:end]
                sub_profiles = batch_user_profiles[start:end] if batch_user_profiles else None
                results_per_chunk[idx] = self._batch_call_single(url, sub_histories, sub_turn_ids, sub_statuses, sub_profiles, max_retries)
            except Exception as e:
                logger.error(f"Instance {url} failed: {e}")
                results_per_chunk[idx] = self._create_error_responses(
                    end - start, sub_turn_ids, sub_statuses,
                    error_msg=f"Instance {url} failed: {e}"
                )

        threads = []
        for idx, (url, start, end) in enumerate(chunks):
            t = threading.Thread(target=_call_instance, args=(idx, url, start, end))
            t.start()
            threads.append(t)
        for t in threads:
            t.join()
        ...
        for idx in range(len(chunks)):
            if results_per_chunk[idx] is not None:
                merged.extend(results_per_chunk[idx])
            else:
                _, start, end = chunks[idx]
                merged.extend(self._create_error_responses(
                    end - start, turn_ids[start:end], batch_statuses[start:end],
                    error_msg=f"Instance {urls[idx]} returned no results"
                ))

        while len(merged) < batch_size:
            idx = len(merged)
            merged.append({'response': '', 'status_value': 0, 'turn_id': turn_ids[idx], 'batch_statuses': batch_statuses[idx], '_error': 'Missing response'})
        return merged[:batch_size]

    def _batch_call_single(self, server_url: str, histories: List[List[Dict[str, str]]], turn_ids: List[int], batch_statuses: List[List[int]], batch_user_profiles: List[Dict], max_retries: int) -> List[Dict[str, Any]]:
        """Call a single user simulator instance (with automatic restart)."""
        ...
        batch_payload = {
            ...
            'batch_user_profiles': batch_user_profiles,
            'mode': vllm_mode,
        ...
        for attempt in range(max_retries):
            try:
                logger.info(f"[{server_url}] Attempt {attempt + 1}/{max_retries} ({batch_size} samples)...")
                ...
                if resp.status_code == 200:
                    ...
                    logger.info(f"✅ [{server_url}] API call successful (attempt {attempt + 1}): Gen={gen_time:.2f}s, Total={total_time:.2f}s")
                    
                    return results
                
                else:
                    logger.warning(f"⚠️ [{server_url}] HTTP {resp.status_code} (attempt {attempt + 1}/{max_retries})")
                ...
                logger.error(f"❌ [{server_url}] Request timeout (attempt {attempt + 1}/{max_retries})")
                ...
                logger.error(f"❌ [{server_url}] Connection error: {e} (attempt {attempt + 1}/{max_retries})")
                ...
                logger.error(f"❌ [{server_url}] Unexpected error: {e} (attempt {attempt + 1}/{max_retries})")
                logger.error(traceback.format_exc())
                
                if attempt < max_retries - 1:
                    time.sleep(5)
        
        logger.error(f"❌ [{server_url}] All {max_retries} attempts failed")
        return self._create_error_responses(batch_size, turn_ids, batch_statuses, error_msg=f"Service {server_url} unavailable after {max_retries} retries")
    ...
    def run_llm_loop(self, gen_batch: DataProto, initial_input_ids: torch.Tensor) -> DataProto:
        ...
        user_profiles = self._get_batch_user_profiles(gen_batch, batch_size)
        ...
        for step in range(self.config.max_turns):
            ...
            for i, (is_active, is_valid) in enumerate(zip(active_mask.tolist(), valid_flags)):
                ...
                if is_valid:
                    ...
                    if cached_response:
                        ...
                        call_user_profiles.append(user_profiles[i])
            ...
            if call_indices:
                ...
                api_results = self._batch_call_chatbot(self.config.chatbot_url, call_histories, call_turn_ids, call_batch_statuses, call_user_profiles)
        ...
        # Save rollout data
        try:
            save_dir = os.path.join(os.environ['STORAGE_PATH'], 'temp_dialog')
