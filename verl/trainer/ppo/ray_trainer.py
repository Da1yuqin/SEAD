...
import re
from SEAD.llm_agent.generation import LLMGenerationManager, GenerationConfig
...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        gen_config = GenerationConfig(
            ...
            chatbot_url = self.config.chat_server.url,
            request_timeout_s = self.config.chat_server.request_timeout_s,
            mode = self.config.get('generation_mode', 'call_chatbot'),  # get mode from config
        )
        ...
        generation_manager = LLMGenerationManager(
            ...
            config=gen_config,
            is_validation = True,
            use_mock = self.config.chat_server.use_mock,
        )

        if not self.config.multi_turn_chat:
            ...
            for batch_dict in self.val_dataloader:
                ...
                test_gen_batch.non_tensor_batch = test_batch.non_tensor_batch  # preserve scenario_id etc. for generation
                test_gen_batch.meta_info = {
                    'eos_token_id': self.tokenizer.eos_token_id,
                    'pad_token_id': self.tokenizer.pad_token_id,
                    'recompute_log_prob': False,
    ...
    def fit(self):
        ...
        gen_config = GenerationConfig(
            ...
            chatbot_url = self.config.chat_server.url,
            request_timeout_s = self.config.chat_server.request_timeout_s,
            mode = self.config.get('generation_mode', 'call_chatbot'),  
        )
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                gen_batch.non_tensor_batch = batch.non_tensor_batch  # preserve scenario_id etc. for generation
                ...
                # with _timer('step', timing_raw):
                    ...
                    if self.config.trainer.critic_warmup <= self.global_steps:
                        ...
                        with _timer('update_actor', timing_raw):
                            if self.config.multi_turn_chat and self.config.actor_rollout_ref.actor.state_masking:
