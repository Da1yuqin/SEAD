...
def _parse_ground_truth_history(ground_truth_history: List[Dict]) -> Dict:
    ...
    Handles chatbot-initiated hangup and ensures model_statuss
    and raw_statuss are correctly paired.
    """
    result = {
        "turns": [],
        "raw_statuss": [],
    ...
    for i, item in enumerate(ground_truth_history):
        ...
        if role == 'assistant':
            ...
            if current_turn and current_turn.get('user_reply'):
                ...
                # Only pair when both model_status and user_status exist
            ...
            # If this is the last message (chatbot-initiated hangup)
            ...
            # If there is no current_turn yet, create one
            if not current_turn:
                turn_id += 1
                current_turn = {'turn_id': turn_id}
    ...
    # Handle the last turn (if not yet saved)
    if current_turn and current_turn.get('user_reply'):
        result["turns"].append(current_turn)
        if current_turn.get('model_status') is not None and current_turn.get('user_status') is not None:
            result["model_statuss"].append(current_turn['model_status'])
...
class ImprovedMultiTurnDialogueReward:
    ...
    def _get_trajectory_completion_status(self, trajectory_data: Dict) -> int:
        ...
        # Traverse backwards to find the last turn with user_status
        for turn in reversed(turns):
            status = turn.get("user_status")
            if status is not None:
                return status
        
        # If no user_status found, return 0
        return 0
    ...
    def compute_chatbot_reward(self) -> Dict[str, float]:
        ...
        # Statistics: based on user_status (actual completion)
        for traj in self.trajectories:
            user_status = self._get_trajectory_completion_status(traj)
            if user_status == 1:
                completed_count += 1
            
            # Extract model_status (Chatbot's prediction)
            ...
            # Count FP: Chatbot predicted complete, but User did not agree
            if model_status == 1:
        ...
        for i in range(total_count):
            ...
            # Use user_status to determine actual completion
            user_status = self._get_trajectory_completion_status(traj)
            ...
            # Only penalize when user did not agree (user_status != 1) and model hung up (model_status == 1)
            if model_status == 1 and user_status != 1:
                fpr_penalty = -10.0  # Severe penalty for false positive
            else:
                fpr_penalty = 0.0

            # 2. Turn Bonus (multi-turn bonus)
            # Encourage the model to have more turns, 0.1 per turn, max 1.5 (15 turns)
            # Only awarded when there is no false positive hangup
            if fpr_penalty == 0:
                turn_bonus = min(num_turns * 0.1, 1.5)
            else:
                turn_bonus = 0.0  # No turn bonus on false positive hangup

            # 3. Efficiency Bonus (efficiency/success bonus)
            # Large bonus for successful completion
            if individual_completion == 1.0:
                completion_reward = 10.0  # 10 points for successful completion
                # Extra bonus if model correctly hung up
                if model_status == 1:
                    efficiency_bonus = 2.0  # 2 extra points for correct hangup
                else:
            ...
            # Reward formula
            # Total = completion + state_transition + turn_bonus + efficiency + penalty
            traj_total = (
                completion_reward + 0
                # 0.5 * traj_state_transition +  # state transition weight 0.5
                # turn_bonus + 
                # efficiency_bonus +
        ...
        print(f"\n[Trajectory Level Reward Details (Total {total_count}):]")
...
class ChatbotMetricCalculator:
    ...
    def _check_stage_completeness(self, trajectory_data: Dict) -> bool:
        ...
        Example:
        - [0, 1, 2, 3] -> Complete
        - [0, 1, 2] -> Complete (Last is 2, preceded by 0 and 1)
        - [0, 2, 3] -> Incomplete (Last is 3, but missing 1)
        - [1, 2, 3] -> Incomplete (Missing 0)
        - [0, 1] -> Complete (Last is 1, preceded by 0)
        """
        turns = trajectory_data.get("turns", [])
...
def compute_score_chatbot(ground_truth_histories: List[List[Dict]],
    ...
    if file_path:
        try:
            ...
            print(f"[OK] Results saved to: {versioned_path}\n")
...
def compute_metric_chatbot(ground_truth_histories: List[List[Dict]],
    ...
    true_positive_count = 0   # TP: Chatbot predicted complete, User agreed
    false_positive_count = 0  # FP: Chatbot predicted complete, User disagreed (false positive)
    negative_count = 0
    ...
    for i, ground_truth_history in enumerate(ground_truth_histories):
        ...
        # Extract states from parsed data
        ...
        if model_statuss and raw_statuss:
            # Take the last status (ensure both lists have consistent length)
        ...
        if (i + 1) % 10 == 0 or i == 0:
            print(f"[OK] Parsed {i + 1}/{len(ground_truth_histories)} trajectories")
    ...
    fpr_rate = false_positive_count / (false_positive_count + true_positive_count) if (false_positive_count + true_positive_count) > 0 else 0.0
    
    # Also compute other useful metrics
    actual_positive_count = true_positive_count + false_positive_count
    ...
    # 4. Print metrics
    print(f"\n{'='*80}")
    print(f"[Chatbot Test Set Metrics (Step {current_step})]")
    print(f"{'='*80}\n")
    
    print(f"[L1: Process Compliance]")
    ...
    print(f"  False Positive Rate (FPR):        {fpr_rate:.2%} ({false_positive_count}/{false_positive_count + true_positive_count})")
    ...
    if metrics['emotion_variation_mean'] is not None:
        if metrics['emotion_variation_std'] is not None:
            print(f"  Emotion Trajectory Variation (ETV): {metrics['emotion_variation_mean']:+.3f} +/- {metrics['emotion_variation_std']:.3f}")
        else:
    ...
    if metrics['profiling_accuracy'] is not None:
        if metrics['profiling_accuracy_std'] is not None:
            print(f"  User Profiling Accuracy:          {metrics['profiling_accuracy']:.4f} +/- {metrics['profiling_accuracy_std']:.4f}")
        else:
    ...
    if metrics['trust_variation_mean'] is not None:
        if metrics['trust_variation_std'] is not None:
            print(f"  Trust Variation:                  {metrics['trust_variation_mean']:+.3f} +/- {metrics['trust_variation_std']:.3f}")
        else:
    ...
    if metrics['cooperation_variation_mean'] is not None:
        if metrics['cooperation_variation_std'] is not None:
            print(f"  Cooperation Variation:            {metrics['cooperation_variation_mean']:+.3f} +/- {metrics['cooperation_variation_std']:.3f}")
        else:
    ...
    try:
        ...
        print(f"[FAIL] Failed to save metrics: {e}\n")
    ...
    if file_path:
        try:
            ...
            print(f"[OK] Trajectory details saved to: {versioned_path}\n")
        except Exception as e:
            print(f"[FAIL] Failed to save trajectory details: {e}\n")
