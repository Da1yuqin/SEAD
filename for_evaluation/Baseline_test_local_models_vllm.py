# for_evaluation/Baseline_test_local_models_vllm.py
"""
...
# add project root directory automatically
...
try:
    ...
    print("WARNING: vLLM not installed, will use transformers (slower)")
...
class UserParamsManager:
    ...
    def _load_user_params(self, split: str) -> list:
        ...
        if not file_path.exists():
            print(f"WARNING: {file_path} not found, using empty params")
            return []
    ...
    def get_batch_user_params(self, batch_size: int) -> list:
        ...
        if len(params_list) == 0:
            print(f"WARNING: {split} user_params is empty, using empty dicts")
            return [{}] * batch_size
...
def ensure_vllm_service_available(
    ...
    for attempt in range(max_retries):
        ...
        if healthy:
            if unhealthy:
                print(f"WARNING: Some services abnormal: {len(unhealthy)}/{len(server_urls)}")
            return healthy
        ...
        if attempt < max_retries - 1:
            ...
            if not success:
                print(f"WARNING: Restart failed, waiting 30 seconds before retry...\n")
                time.sleep(30)
        else:
...
def load_local_model_vllm(
    ...
    print(f"  - VLLM_USE_V1: {v1_status} {'(V0 engine)' if v1_status == '0' else 'WARNING: (V1 engine)'}")
    print()
...
def call_local_model_batch_vllm(
    ...
    batch_user_profiles: List[Dict] = None
) -> List[Dict]:
    """
    Batch inference using vLLM (high-performance version)
    Supports user_params filling
    """
    global LOCAL_MODEL, LOCAL_TOKENIZER, DEBUG_PRINT_COUNT
    ...
    try:
        # 1. Use caller-supplied user_profiles (bound to each sample via parquet)
        if batch_user_profiles is not None and len(batch_user_profiles) == batch_size:
            batch_user_params = batch_user_profiles
        else:
...
def call_local_model_batch(batch_messages, temperature=0.7, max_new_tokens=512, batch_user_profiles=None):
    """Automatically select vLLM or transformers"""
    if VLLM_AVAILABLE and isinstance(LOCAL_MODEL, LLM):
        return call_local_model_batch_vllm(batch_messages, temperature, max_new_tokens, batch_user_profiles=batch_user_profiles)
    else:
        return call_local_model_batch_transformers(batch_messages, temperature, max_new_tokens, batch_user_profiles=batch_user_profiles)
...
def call_batch_user_simulator(
    ...
    batch_size = len(batch_dialogue_histories)
    if batch_user_profiles is None or len(batch_user_profiles) != batch_size:
        raise ValueError(
            f"batch_user_profiles must be provided and match batch_size ({batch_size}), "
            f"got: {None if batch_user_profiles is None else len(batch_user_profiles)}"
        )
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            print(f"  WARNING: User Sim Error: {resp.status_code} (URL: {server_url})")
            return [default_fail] * batch_size
    
    except requests.exceptions.Timeout:
        print(f"  WARNING: User Sim Timeout: {timeout}s (URL: {server_url})")
        return [default_fail] * batch_size
    except requests.exceptions.ConnectionError:
        print(f"  WARNING: User Sim Connection Error (URL: {server_url})")
        return [default_fail] * batch_size
    except Exception as e:
        print(f"  WARNING: User Sim Exception: {e} (URL: {server_url})")
        return [default_fail] * batch_size
...
class ConversationState:
    ...
    def __init__(self, idx, initial_prompt, user_profile=None):
        self.idx = idx
...
def process_batch(
    ...
    """
    all_conversations = [ConversationState(idx, prompt, user_profile) for idx, prompt, user_profile in batch_data]
    ...
    for turn in range(max_turns):
        ...
        # Batch call Chatbot (pass per-sample user_profiles from parquet)
        batch_messages = [conv.chatbot_messages for conv in active_conversations]
        batch_profiles = [conv.user_profile for conv in active_conversations]
        batch_results = call_local_model_batch(batch_messages, batch_user_profiles=batch_profiles)
            ...
            if conv.chatbot_status == 1:
                ...
                if conv.batch_statuses:
                    ...
                    if last_user_status == 1:
                        ...
                        print(f"    [Sample {conv.idx}] [Turn {turn+1}] Chatbot hung up (user not agreed, misjudgment)")
                else:
        ...
        batch_user_profiles = [c.user_profile for c in sim_candidates]

        print(f"  > Calling User Simulator: {len(sim_candidates)} samples (using {len(user_sim_urls)} instances)")
        for c in sim_candidates:
            p = c.user_profile
            behaviors_str = '; '.join(p.get('behaviors', [])) or '(none)'
            print(
                f"    [Sample {c.idx}] [Turn {turn+1}] "
                f"coop={p.get('cooperation')} emo={p.get('emotion')} trust={p.get('trust')} | "
                f"behaviors: {behaviors_str}"
            )
        ...
        for i, conv in enumerate(sim_candidates):
            ...
            # 3. Timeout check
                ...
                conv.stats['total_turns'] = turn + 1
                conv.is_finished = True
                print(f"    [Sample {conv.idx}] [Turn {turn+1}] Conversation timeout (incomplete)")
...
def test_model_on_dataset(
    ...
    if restart_config:
        ...
        print(f"WARNING: Auto-restart feature: Disabled")
    ...
    for url in user_sim_urls:
        try:
            ...
            if resp.status_code == 200:
                print(f"  OK: {url} - Normal")
                available_urls.append(url)
            else:
                print(f"  WARNING: {url} - Abnormal (status code: {resp.status_code})")
        except Exception as e:
            print(f"  ERROR: {url} - Cannot connect ({e})")
    
    if not available_urls:
        if restart_config:
            print("\nWARNING: No available User Simulator services, attempting auto-restart...")
            try:
                available_urls = ensure_vllm_service_available(
                    server_urls=user_sim_urls,
                    restart_script=restart_config['restart_script'],
            ...
            print("\nERROR: No available User Simulator services!")
            return None
    ...
    if 'user_profile' not in test_data.columns:
        raise ValueError(
            "test_data is missing 'user_profile' column. "
            "Please regenerate the parquet with utils/create_prompt_data.py."
        )

    data_list = []
    for idx, row in test_data.iterrows():
        raw_profile = row['user_profile']
        if isinstance(raw_profile, dict):
            ...
            raise ValueError(
                f"sample {idx}: expected dict or JSON str for user_profile, got {type(raw_profile)}"
            )
        data_list.append((idx, row['prompt'][0]['content'], user_profile))
...
def main():
    ...
    try:
        ...
        print(f"WARNING: Unable to load user_params: {e}")
    ...
    if not args.disable_auto_restart and args.vllm_model_path:
        ...
        print(f"\n{'='*80}")
        print("WARNING: Auto-restart feature disabled")
        print(f"{'='*80}\n")
    ...
    try:
        if VLLM_AVAILABLE and not args.use_transformers:
            ...
            if args.use_transformers:
                print("WARNING: Using --use_transformers parameter, will use Transformers engine")
